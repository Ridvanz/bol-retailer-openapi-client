...
class SubscriptionRequest(BaseModel):
    ...
    SubscriptionRequest
    """ # noqa: E501
    resources: List[StrictStr] = Field(description="Array of event types for which the subscription is set. Note that some event types are only available for certain subscription types.")
    ...
    __properties: ClassVar[List[str]] = ["resources", "url", "subscriptionType", "enabled"]

    @field_validator('resources')
    def resources_validate_enum(cls, value):
        """Validates the enum"""
        for i in value:
            if i not in set(['PROCESS_STATUS', 'SHIPMENT', 'OFFER', 'PRICE_STAR_BOUNDARY', 'COMPETING_OFFER']):
                raise ValueError("each list item must be one of ('PROCESS_STATUS', 'SHIPMENT', 'OFFER', 'PRICE_STAR_BOUNDARY', 'COMPETING_OFFER')")
        return value
